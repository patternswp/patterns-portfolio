/*CSS*/
/* global PatternsPortfolioLocalize */
...
class PatternsPortfolioSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-portfolio-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-portfolio-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-portfolio-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-portfolio-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patterns-portfolio-processing' );
					this.installPlugins();
				}
			}
		} );
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-portfolio-review-notice'
			);
