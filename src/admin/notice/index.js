...
/* Local */
class PatternsPortfolioSettingsApi {
    constructor() {
        this.endpoint = 'patterns-portfolio/v1/settings';
    ...
    bindEvents() {
        document.addEventListener('click', (event) => {
            if (event.target.classList.contains('patterns-portfolio-gsn-close')) {
                this.modifySettings('hide_get_started_notice', true);
            }
            else if (event.target.classList.contains('patterns-portfolio-rn-permanent-close')) {
                this.modifySettings('remove_review_notice_permanently', true);
            }
            else if (event.target.classList.contains('patterns-portfolio-rn-temporary-close')) {
                this.modifySettings('remove_review_notice_temporary_date_time', 1);
            }
            else if (event.target.classList.contains('patterns-portfolio-install-plugins')) {
    ...
    updateUI() {
        if (this.settings.hide_get_started_notice) {
            const noticeElement = document.getElementById('patterns-portfolio-gsn');
            if (noticeElement) {
        ...
        if (!this.isLoadReview()) {
            const reviewNoticeElement = document.getElementById('patterns-portfolio-review-notice');
    ...
    async installPlugins() {
        const plugins = PatternsPortfolioLocalize.recommended_plugins;
        ...
        window.location.replace(PatternsPortfolioLocalize.theme_info_url);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new PatternsPortfolioSettingsApi();
});
