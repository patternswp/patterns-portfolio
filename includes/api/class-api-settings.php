 ...
 * @subpackage Patterns_Portfolio/Patterns_Portfolio_Api_Settings
 */
 ...
 *
 * @see Patterns_Portfolio_Api
 */
if ( ! class_exists( 'Patterns_Portfolio_Api_Settings' ) ) {

	/**
	 * Patterns_Portfolio_Api_Settings
	 *
	 * @see WP_REST_Settings_Controller
	 * @package Patterns_Portfolio
	 * @since 1.0.0
	 */
	class Patterns_Portfolio_Api_Settings extends Patterns_Portfolio_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'patterns_portfolio_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = patterns_portfolio_get_options();
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'patterns-portfolio' ), PATTERNS_PORTFOLIO_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}
			...
			/* Update options */
			patterns_portfolio_update_options(
				array(
			...
			/* Update user meta */
			patterns_portfolio_update_user_meta(
				get_current_user_id(),
				array(
		...
		public function get_settings_schema() {
			$setting_properties = apply_filters(
				'patterns_portfolio_options_properties',
				array(
...
/**
 * Return instance of  Patterns_Portfolio_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return Patterns_Portfolio_Api_Settings
 */
function patterns_portfolio_api_settings() { //phpcs:ignore
	return Patterns_Portfolio_Api_Settings::get_instance();
}
patterns_portfolio_api_settings()->run();
