<?php // phpcs:ignore
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * Patterns Portfolio functions and definitions
 *
 * @link https://developer.wordpress.org/themes/basics/theme-functions/
 *
 * @package Patterns Portfolio
 */

/**
 * Current theme path.
 * Current theme url.
 * Current theme version.
 * Current theme name.
 * Current theme option name.
 */
define( 'PATTERNS_PORTFOLIO_PATH', trailingslashit( get_template_directory() ) );
define( 'PATTERNS_PORTFOLIO_URL', trailingslashit( get_template_directory_uri() ) );
define( 'PATTERNS_PORTFOLIO_VERSION', '1.0.0' );
define( 'PATTERNS_PORTFOLIO_THEME_NAME', 'patterns-portfolio' );
define( 'PATTERNS_PORTFOLIO_OPTION_NAME', 'patterns-portfolio' );

/**
 * The core theme class that is used to define internationalization,
 * admin-specific hooks, and public-facing site hooks.
 */
require PATTERNS_PORTFOLIO_PATH . 'includes/main.php';

/**
 * Begins execution of the theme.
 *
 * @since    1.0.0
 */
function patterns_portfolio_run() {
	new Patterns_Portfolio();
}
patterns_portfolio_run();
